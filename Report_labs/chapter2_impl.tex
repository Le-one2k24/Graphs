...
Классический алгоритм Дейкстры реализован с проверкой на наличие отрицательных весов (в этом случае выводится сообщение об ошибке). Расстояния хранятся в массиве $dist$, родители — в $parent$. На каждом шаге выбирается непосещённая вершина с минимальным расстоянием, затем обновляются все исходящие рёбра.
...
Для вычисления самого определителя используется отдельная функция $determinant\_ld()$

\noindent\textbf{Вход:}
\begin{enumerate}
	\item vector<vector<long double>>\& A — подматрица матрицы Кирхгофа (получена путём удаления последней строки и последнего столбца)
	\item int n — количество строк/столбцов подматрицы
\end{enumerate}
\noindent\textbf{Выход:}
long double — определитель подматрицы

\begin{lstlisting}[caption=Вспомогательная функция вычисления определителя (kirchhoff.cpp)]
...
\noindent\textbf{Вход:}
const Cut\& a, const Cut\& b — два разреза
\noindent\textbf{Выход:}
...
\noindent\textbf{Вход:}
const vector<Cut>\& cuts — список разрезов
\noindent\textbf{Выход:}
...
В главном файле \texttt{main.cpp} реализовано текстовое меню, позволяющее пользователю последовательно вызывать все реализованные алгоритмы. После генерации графа все данные сохраняются в глобальных переменных, что позволяет выполнять над ними различные операции без повторной генерации. Меню поддерживает следующие действия:

\begin{itemize}
	\item генерация ориентированного связного ациклического графа по степеням распределения Чампернауна;
	\item преобразование ориентированного графа в неориентированный (симметризация матриц);
	\item генерация весовой матрицы для текущего графа с выбором типа весов (положительные, отрицательные, смешанные);
	\item вывод текущего графа и его метрик (эксцентриситеты вершин, центр, диаметр);
	\item отображение сохранённой весовой матрицы;
	\item метод Шимбелла для нахождения минимальных и/или максимальных путей с ограничением по числу рёбер;
	\item подсчёт количества маршрутов между двумя заданными вершинами;
	\item поиск точек сочленения в текущем графе (только для неориентированного графа);
	\item нахождение кратчайшего пути между двумя вершинами классическим алгоритмом Дейкстры;
	\item генерация матриц пропускных способностей и стоимостей для текущего графа;
	\item отображение матриц пропускных способностей и стоимостей;
	\item нахождение максимального потока алгоритмом Эдмондса–Карпа;
	\item нахождение потока минимальной стоимости объёма $\left\lfloor \frac{2}{3} \cdot \text{max} \right\rfloor$;
	\item вычисление числа остовных деревьев по матричной теореме Кирхгофа для текущего неориентированного графа;
	\item построение минимального остовного дерева (алгоритм Борувки) и сохранение его как последнего MST;
	\item кодирование последнего MST в код Прюфера и декодирование обратно с сохранением весов рёбер;
	\item нахождение минимальной раскраски графа (на исходном графе или на последнем MST);
	\item проверка графа на эйлеровость, при необходимости модификация и построение эйлерова цикла;
	\item построение фундаментальной системы разрезов последнего MST и получение симметрической разности выбранных разрезов;
...
ввод пользователя \\
\noindent\textbf{Выход:}
выполнение выбранных операций и управление консольным приложением
